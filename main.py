...
filename = sys.argv[1]
# holds the binary numbes and thier respective instructions and properties of the instructions
instructions = {
...
instruction_types = {
    'R': {
        'properties': [[21, 0x7FF], [16, 0x1F], [10, 0x3F], [5, 0x1F], [None, 0x1F]],
        'elements': ['opcode', 'rm', 'shamt', 'rn', 'rd'],
    },
    'I': {
        'properties': [[22, 0x3FF], [10, 0xFFF], [5, 0x1F], [None, 0x1F]],
        'elements': ['opcode', 'imm', 'rn', 'rd'],
    },
    'D': {
        'properties': [[21, 0x7FF], [12, 0x1FF], [10, 0x3], [5, 0x1F], [None, 0x1F]],
        'elements': ['opcode', 'address', 'op', 'rn', 'rd'],
    },
    'B': {
        'properties': [[26, 0x3F], [None, 0x3FFFFFF]],
        'elements': ['opcode', 'address'],
    },
    'CB': {
        'properties': [[24, 0xFF], [5, 0x7FFFF], [None, 0x1F]],
        'elements': ['opcode', 'address', 'rt'],
    },
    'IW': {
        'properties': [[21, 0x7FF], [5, 0xFFFF], [None, 0x1F]],
...
def twos_complement_binary_to_integer(binary_str):
    ...
    elif (int(binary_str[0]) & 1) == 0:  # if its positve
        return int(binary_str, 2) #return binary str
    else:  # else, do twos compliment to get positive number
...
def parseInst(instruction, code, inst_num):
    ...
    instruction_properties = instruction_types[instruction_type]['properties']
    instruction_elements = instruction_types[instruction_type]['elements']
    instruction_split = {}
    output = instruction["output"] or ""
    for (j, [shift, mask]) in enumerate(instruction_properties):
        # Get element properties e.g. opcode, address, rt
        key = instruction_elements[j]
        if shift == None:
            # End of properties, but still need to get bits
            instruction_split[key] = bin(int(code, 2) & mask)
        else:
            # Use properties array to fetch respective bits for key
        ...
        if (instruction['name'] == "Bcond" and (key == "rt" or key == "address")):
            ...
            # Check for common registers and replace if needed
